...
test('It works without config', function(assert) {
  this.application.registry.register('config:environment', { APP: {}, embedded: true }, { instantiate: false });
...
test('It merges the embedded config back to the App Config', function(assert) {
  this.application.registry.register('config:environment', { APP: {}, embedded: true }, { instantiate: false });
  this.application.registry.register('config:embedded', { yoKey: 'Yo Value!' }, { instantiate: false });
  initialize(this.appInstance);

  assert.equal(this.appInstance.container.lookup('config:environment').APP.yoKey, 'Yo Value!');
});
